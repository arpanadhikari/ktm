document.addEventListener("DOMContentLoaded", () => {
  ...
  // Call the function to update the dashboard initially
  updateDashboard();
...
async function mapPodsToNodes() {
    ...
    //extract node data from cluster snapshot with the following format:
    /*
    { data : [{poddata},{nodedata}] }
    */
...
async function getClusterSnapshot(relativeTime) {
  const response = await fetch('/clustersnapshot?relativeTime=' + relativeTime);
  const data = await response.json();
  console.log("Response:", data);
  return data;
}
...
async function updateDashboard() {
  ...
  .attr("href", (d) => {
    ...
    return d.height === 1 ? "nodehistory/"+d.data.name+"?relativeTime=10h" : "podhistory/"+d.data.name+"?relativeTime=10h"
    return d.data.name+"/"
